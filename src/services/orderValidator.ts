import { OrderItem, ItemVariation, Attribute, Variation } from '@/types';
...
export class OrderValidator {
  static validateItem(
    ...
    const selectedAttributeIds = new Set(
      item.item_variations.map((v) => v.attribute_id)
    );
    ...
    for (const variation of item.item_variations) {
      const isValid = availableVariations.some(
        (v) => v.id === variation.variation_id && v.attribute_id === variation.attribute_id
      );
  ...
  static calculateItemTotal(item: OrderItem): number {
    const variationsTotal = item.item_variations.reduce(
      (sum, v) => sum + v.extra_price,
      0
    );
    const extrasTotal = item.item_extras.reduce(
  ...
  static normalizeItemForOrder(item: Partial<OrderItem>, branchId: number): OrderItem {
    const variationTotal = (item.item_variations || []).reduce(
      (sum, v) => sum + v.extra_price,
      0
    );

    const extraTotal = (item.item_extras || []).reduce(
      (sum, e) => sum + e.price * e.qty,
      0
    );
