...
import { OrderType, PosPaymentMethod } from '../types';
import env from '../config/env';

async function dryRunCreatePosOrder() {
  try {
    logger.info('Starting dry run for POS order creation...');

    await laravelClient.login();
    logger.info('✓ Successfully authenticated with Laravel backend');

    const customerPhone = '+15555551234';
    logger.info({ phone: customerPhone }, 'Searching for existing customer...');

    let customers = await laravelClient.searchCustomer(customerPhone);
    let customer;

    if (customers.length === 0) {
      logger.info('Customer not found, creating new customer...');
      customer = await laravelClient.createCustomer({
      ...
      logger.info({ customerId: customer.id }, '✓ Found existing customer');
    }
    ...
    logger.info('Creating delivery address...');
    const address = await laravelClient.createAddress(customer.id, deliveryAddress);
    logger.info({ addressId: address.id }, '✓ Created address');

    logger.info('Fetching menu for branch 1...');
    const menu = await laravelClient.getMenu(1);
    logger.info({ itemCount: menu.length }, '✓ Fetched menu');
    ...
    logger.info({ itemId: firstItem.id, itemName: firstItem.name }, 'Selected menu item');

    logger.info('Fetching variations for item...');
    const variations = await laravelClient.getVariations(firstItem.id);
    logger.info({ variationCount: variations.length }, '✓ Fetched variations');
    ...
    logger.info({ orderData: { ...orderData, items: '[ITEMS]' } }, 'Creating POS order...');
    ...
    console.log(`Source: ${env.AI_ORDER_SOURCE} (AI Orders)`);
    console.log('========================================\n');
  } catch (error) {
