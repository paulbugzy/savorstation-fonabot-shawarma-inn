...
const branchDetection_1 = __importDefault(require("./branchDetection"));
...
class AIAgent {
    async processUserInput(session, userMessage) {
        ...
        try {
            ...
            logger_1.default.error({ error }, 'OpenAI API error');
            return {
    ...
    buildMessages(session, userMessage) {
        ...
        messages.push(...session.conversationHistory.map((msg) => ({
            role: msg.role,
            content: msg.content,
        })));
    ...
    buildContextString(session) {
        ...
        if (session.orderType) {
            const typeNames = {
                [types_1.OrderType.DELIVERY]: 'Delivery',
                [types_1.OrderType.TAKEAWAY]: 'Pickup',
                [types_1.OrderType.DINING_TABLE]: 'Dine-In',
            };
            parts.push(`Order Type: ${typeNames[session.orderType]}`);
        }
        if (session.branchId) {
            const branchNames = {
                [types_1.BranchId.CHICAGO]: 'Chicago',
                [types_1.BranchId.PARK_RIDGE]: 'Park Ridge',
            };
    ...
    async executeToolCall(toolCall, session) {
        const { name, arguments: argsStr } = toolCall.function;
        const args = JSON.parse(argsStr);
        logger_1.default.info({ name, args }, 'Executing tool call');
        try {
            switch (name) {
                case 'searchCustomer':
                    return await laravelClient_1.default.searchCustomer(args.query);
                case 'createCustomer':
                    const customer = await laravelClient_1.default.createCustomer({
                        name: args.name,
                        phone: args.phone,
                        email: args.email,
                ...
                case 'createAddress':
                    const address = await laravelClient_1.default.createAddress(args.customerId, {
                        line1: args.line1,
                        city: args.city,
                        state: args.state,
                        zip: args.zip,
                ...
                case 'getMenu':
                    return await laravelClient_1.default.getMenu(args.branchId);
                case 'getVariations':
                    return await laravelClient_1.default.getVariations(args.itemId);
                case 'detectBranch':
                    const branch = await branchDetection_1.default.detectBranchForDelivery(args.address);
                    return {
                        success: true,
                        branchId: branch.id,
                ...
                case 'setOrderType':
                    const orderTypeMap = {
                        delivery: types_1.OrderType.DELIVERY,
                        pickup: types_1.OrderType.TAKEAWAY,
                        'dine-in': types_1.OrderType.DINING_TABLE,
                    };
                ...
                case 'setBranch':
                    const branchMap = {
                        chicago: types_1.BranchId.CHICAGO,
                        'park ridge': types_1.BranchId.PARK_RIDGE,
                    };
                    ...
                    return {
                        ...
                        reducerUpdate: {
                            ...
                            payload: {
                                ...
                                item_variations: (args.item.item_variations || []).map((variation) => ({
                                ...
                                item_extras: (args.item.item_extras || []).map((extra) => ({
                ...
                case 'setPayment':
                    const paymentMap = {
                        cash: types_1.PosPaymentMethod.CASH,
                        card: types_1.PosPaymentMethod.CARD,
                        mobile: types_1.PosPaymentMethod.MOBILE_BANKING,
                        other: types_1.PosPaymentMethod.OTHER,
                    };
                    ...
                    return {
                        ...
                        reducerUpdate: {
                            ...
                            payload: {
                                ...
                                note: args.note,
                                amount: args.amount !== undefined && args.amount !== null
                                    ? Number(args.amount)
                                    : undefined,
                            },
                        },
            ...
            logger_1.default.error({ error, toolName: name }, 'Tool execution failed');
            return { success: false, error: error.message };
        }
    }
}
exports.AIAgent = AIAgent;
exports.default = new AIAgent();
